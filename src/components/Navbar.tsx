    ...
    <header className="sticky top-0 z-50 px-4 py-3 max-w-7xl mx-auto w-full transition-all">
      <div className="glass-panel rounded-3xl px-5 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2.5">
          {/* Price Badge Pill */}
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-800 text-[11px] font-bold">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span>
            <span>18-Month Plan: 649 ETB</span>
          </div>

          {/* AI Sales Chat Button */}
          <button
            onClick={onOpenChat}
            className="flex items-center gap-1.5 min-h-[40px] px-3.5 py-2 rounded-full text-xs font-bold text-blue-700 bg-blue-50/90 hover:bg-blue-100 border border-blue-200/60 active:scale-95 transition-all cursor-pointer select-none"
          ...
          {user && (
            <button
              ...
              className="relative flex items-center gap-1.5 min-h-[40px] px-3.5 py-2 rounded-full text-xs font-semibold text-[#111827] bg-white/80 hover:bg-white border border-white shadow-xs active:scale-95 transition-all cursor-pointer select-none"
          ...
          {user ? (
            <div className="flex items-center gap-2 pl-1">
              <div className="flex items-center gap-1.5 bg-white/60 rounded-full py-1.5 px-3 border border-white/80">
                {user.photoURL ? (
                ...
                <button
                  onClick={onLogout}
                  title="Sign Out"
                  className="text-[#9CA3AF] hover:text-rose-600 p-1 rounded-full transition-colors cursor-pointer min-w-[28px] min-h-[28px] flex items-center justify-center"
                >
            ...
            <button
              onClick={onOpenAuth}
              className="clay-button min-h-[40px] px-4 py-2 text-xs font-bold text-[#111827] flex items-center gap-1.5 cursor-pointer select-none active:scale-95"
            >
          ...
          <button
            ...
            className="clay-button min-h-[40px] px-4 py-2 text-xs font-bold text-[#111827] flex items-center gap-2 cursor-pointer bg-gradient-to-r from-blue-50 via-indigo-50 to-cyan-50 select-none active:scale-95"
          >
